...
void app_main(void)
{
    ...
    node_identity_resolve(NODE_ROLE_WORMHOLE_B);
...
void app_main(void)
{
    ...
    /* Identity (nickname) — after the SD boot check, which caches
     * node_config.txt while the card is still mounted. */
    node_identity_resolve(NODE_ROLE_WORMHOLE_A);
    ...
    /* Loud post-flight, mirroring the blackhole attacker's MAC check: if NOTHING
     * ever came down the wire, the wormhole did not happen and this capture
     * cannot show the tunnel signature (TunnelIntensity/TunnelBytes/TunnelLatency
     * all empty) — while both boards still look healthy and log full telemetry.
     *
     * This check belongs on NODE A specifically. On node B a disconnected or
     * mis-wired TX line is UNDETECTABLE: uart_write_bytes() happily succeeds
     * into an unterminated line, so B's "Tunnelled" counter climbs either way.
     * Node A is the only end that can prove a frame actually crossed. */
    if (s_tunnel_received == 0) {
        ...
        ESP_LOGE(TAG, "  COMMON GROUND between the two boards), that both ends were");
        ESP_LOGE(TAG, "  flashed with the matching wormhole roles, and re-run.");
        ESP_LOGE(TAG, "  Bring-up test: the standalone uart_link_test project.");
        ESP_LOGE(TAG, "*********************************************************");
    }
