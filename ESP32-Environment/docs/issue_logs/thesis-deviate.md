...
| **Conventions** | Absent scenario segment ⇒ `scenario = "stationary"`, because the export convention is that the no-variation scenario gets no folder — absence *is* the value, not missing data. **Renamed sep. 24 2026** from `"none"` to `"stationary"` (nodes stay put, no injected variation) **everywhere**: `run.ps1 -Scenario`, wizard/menu, export, import, analysis, checklist, presets, and the firmware constant `TRAFFIC_PROFILE_STATIONARY` (still value 0, so binaries are unchanged). **Stationary now gets its own folder** (`.../<location>/stationary/`) like every other scenario. `"none"` is still accepted as an alias on every input, and a capture from before the rename (no scenario folder) is still read as stationary, so old and new captures pool as one group. Absent location ⇒ `"unrecorded"`, matching the name `run_matrix.py` already used when backfilling `run_ledger.csv`. A path that is not an export tree at all (synthetic fixtures from `generate_fake_data.py`) yields `None` for all four, so unknown provenance is never labelled as if it were known. |
