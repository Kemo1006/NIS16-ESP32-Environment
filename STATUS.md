...
- sep. 21, 2026 — **`docs/EXPECTED-RESULTS.md`** (post-capture checklist with REAL measured numbers + a §0 that explains how to read every number: ratios, dBm, deltas, mu±sd, **z**, n). Fixed the stale attacker-MAC alarms in **`run_wizard.ps1`** (the launcher actually used — the earlier fix only touched `menu.ps1`, which had TWO copies) and added a **PLACEMENT warning**: post-C7 the attacker only drops what transits it, so an attacker at the far end intercepts nothing. Pushed to branch **THESIS3**.
