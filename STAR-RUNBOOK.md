# ⭐ STAR Topology — Full Runbook (Baseline · Blackhole · Wormhole)

> **What this file is:** all three experiments for the **star** topology on **one fixed
> placement** — set up once, swap commands per run. Sibling of
> [`LINEAR-RUNBOOK.md`](LINEAR-RUNBOOK.md) and [`TREE-RUNBOOK.md`](TREE-RUNBOOK.md).
>
> 🏠 **Star wants everything CLOSE to the root.** Star = every node is a **direct child of
> the root** (one hop, no relays). So unlike linear/tree, you *want* the boards **near
> Bedroom 2** (the root). The catch on your floor plan: **Master's Bedroom is the farthest
> room**, so it's the one at risk of going 2 hops — keep an eye on it (see the caveat below).
>
...
Star = a **hub with spokes**. Every board must reach the **root (Bedroom 2) directly** at
strong signal. So cluster the children in the rooms **closest to the root** — Bedroom 1
and the near side of the Family Hall — and keep Master's Bedroom as close to the hall
doorway as you can:
...
```
 ...
 ├─────────────────────┴──────┬──────────────┤
 │   FAMILY HALL              │  WC / shower │
 │   COM25  +  COM22          ├──────────────┤
 │   (near the bedroom wall)  │   MASTER'S   │
 │                            │   BEDROOM    │
 │                            │  COM21 ← keep │
 │                            │  near doorway│
 └────────────────────────────┴──────────────┘
 ...
 Spokes (star):   COM26 ┐
                  COM27 ┤
                  COM25 ┼──► ROOT (COM20, Bedroom 2)   — every board hops DIRECTLY to root
                  COM22 ┤
                  COM21 ┘
```

Which board goes where:
- **Bedroom 2 — COM20 (root)**, the hub, on your laptop.
- **Bedroom 1 — COM26 + COM27** (adjacent to root = strong direct link). Kept together
  because they're the wormhole cable pair — same layout serves all three runs.
- **Family Hall — COM25 + COM22**, placed **near the bedroom-side wall** so they still hit
  the root in one hop.
- **Master's Bedroom — COM21**, placed **near the Family-Hall doorway** (the closest corner
  to the root), not the far terrace end.
...
### 🔑 Placement rules
- ✅ **Close = correct here.** The star's whole point is a single hop to root, so cluster
  the boards toward Bedroom 2 as much as the rooms allow.
- ✅ **All boards must hear the ROOT directly** (not just each other). If
  `verify_topology --expect star` shows a board went **2 hops**, pull it **closer to the
  root** — this is most likely **COM21 in Master's Bedroom** (move it to the hall doorway,
  or worst case into the Family Hall / Bedroom 1).
- ✅ **Tape it down** — reuse for all three runs.
- ⚠️ Channel 11; switch `MESH_CHANNEL` to 1/6 in `mesh_config.h` + re-flash if it drops.

> 📏 **Star + your floor plan:** if Master's Bedroom simply won't hold a **direct** link to
> the root (too far / too many walls), that's expected — either relocate **COM21** nearer
> the root for the star runs, or lower the WiFi TX power isn't the fix here (that makes
> range *shorter*). For star, **shorter distance = better**. Ask me to help if COM21 keeps 2-hopping.
...
| **3. Place & power** | cluster near root, power from banks, let mesh form | 🔋 |
...
4. **Confirm all 7 files are present**, then run M6→M7→M8 → `analysis/<attack>/star_topology/`.
...
attack repeat run `python tools\run_matrix.py --record --topology star --attack <attack> --repeat <N>`.
...
.\run.ps1 -Port COM20 -Role root -Label node1 -Topology star -Wipe -Flash
```
...
```powershell
cd tools
python export_logs.py --port COM20 --role root --label node1 --topology star --attack none --repeat 1
cd ..
```
...
```powershell
Get-ChildItem tools\exports\baseline\star_topology
...
(Get-ChildItem tools\exports\baseline\star_topology\trimmed\*.csv).Count   # MUST be 7 — a short count silently NaNs out PDR/latency
...
```powershell
.\run.ps1 -Port COM20 -Role child -Label node5 -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node6 -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node4 -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node3 -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node2 -Topology star -Wipe -Flash
```
### Phase 3 — Cluster near the root & power all 5 (wait ~30–60 s)
...
```powershell
cd tools
python export_logs.py --port COM20 --role child --label node5 --topology star --attack none --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node6 --topology star --attack none --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node4 --topology star --attack none --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node3 --topology star --attack none --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node2 --topology star --attack none --repeat 1 --delete
cd ..
```
...
python export_logs.py --port COM20 --role root --label node1 --topology star --attack none --repeat 1
cd ..
Get-ChildItem tools\exports\baseline\star_topology      # expect 7 files BEFORE analyzing
python tools\trim_run.py tools\exports\baseline\star_topology             # dry run: lists boot sessions
python tools\trim_run.py tools\exports\baseline\star_topology --apply     # -> ...\star_topology\trimmed\ (raw untouched)
(Get-ChildItem tools\exports\baseline\star_topology\trimmed\*.csv).Count   # MUST be 7 — a short count silently NaNs out PDR/latency
Get-Content tools\exports\baseline\star_topology\trimmed\*_arrivals.csv -TotalCount 1   # MUST list src_mac,seq_num
...
### ✅ Verify
```powershell
cd tools; python validate_integrity.py exports\baseline\star_topology\trimmed; python verify_topology.py --dir exports\baseline\star_topology\trimmed --topology star --attack none --repeat 1 --expect star; cd ..
```
Good = every node a **direct child of root** (layer 2), all rows `0`, steady arrivals, no gaps/dupes.

---

# ▶️ RUN B — BLACKHOLE (star)

COM26 = relay/drop attacker; COM27/COM25/COM22/COM21 = victims. No cable, no MAC editing. Same placement.

> 🔻 **Thesis note (star + blackhole):** proposal Fig 4.17 draws the **ROOT** as the
> attacker, but `root_node` has no packet-drop path, so this testbed keeps **COM26** as the
> attacker (root only announces the phase). Decide before you defend the star-blackhole
> result: **(A)** amend Fig 4.17 to a leaf-attacker, or **(B)** add a drop path to the root
> firmware. Details in [`ATTACKS-Commands.md`](ATTACKS-Commands.md) (star-blackhole DECISION block).
...
.\run.ps1 -Port COM20 -Role child -Label node5 -Attack blackhole -BlackholeRole attacker -Topology star -Wipe -Flash
#   ↳ STA MAC = b0:cb:d8:f3:32:18
.\run.ps1 -Port COM20 -Role child -Label node6 -Attack blackhole -BlackholeRole victim -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node4 -Attack blackhole -BlackholeRole victim -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node3 -Attack blackhole -BlackholeRole victim -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node2 -Attack blackhole -BlackholeRole victim -Topology star -Wipe -Flash
```
### Phase 3 — Same cluster placement & power
### Phase 4 — Boot root LAST
```powershell
Remove-Item -Recurse -Force root_node\build_* -ErrorAction SilentlyContinue
.\run.ps1 -Port COM20 -Role root -Label node1 -Attack blackhole -Topology star -Wipe -Flash -Repeat 1
```
...
```powershell
cd tools
python export_logs.py --port COM20 --role child --label node5 --topology star --attack blackhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node6 --topology star --attack blackhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node4 --topology star --attack blackhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node3 --topology star --attack blackhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node2 --topology star --attack blackhole --repeat 1 --delete
cd ..
```
> 🚨 **Carry each board back to its room but leave it UNPLUGGED.** A child powered
> now logs benign `layer = -1` rows all the way through Phase 7/8 — and those
...
```powershell
cd tools
python export_logs.py --port COM20 --role root --label node1 --topology star --attack blackhole --repeat 1
cd ..
Get-ChildItem tools\exports\blackhole\star_topology      # expect 7 files BEFORE analyzing
...
(Get-ChildItem tools\exports\blackhole\star_topology\trimmed\*.csv).Count   # MUST be 7 — a short count silently NaNs out PDR/latency
...
change `WORMHOLE_UART_*` pins in `mesh_config.h` to a free pair first. Cable stays the whole
run, never to the laptop. Test:
```powershell
cd uart_link_test
...
([cue table](#-when-to-press-ctrl-and-how-to-repeat)). ⏭️ **Skip this phase on r2/r3.**
```powershell
.\run.ps1 -Port COM20 -Role child -Label node5 -Attack wormhole -WormholeEnd A -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node6 -Attack wormhole -WormholeEnd B -Topology star -Wipe -Flash
.\run.ps1 -Port COM20 -Role child -Label node4 -Topology star -Wipe -Flash    # control
.\run.ps1 -Port COM20 -Role child -Label node3 -Topology star -Wipe -Flash    # control
.\run.ps1 -Port COM20 -Role child -Label node2 -Topology star -Wipe -Flash    # control
```
### Phase 3 — Same cluster placement; keep A↔B cable connected in Bedroom 1; power all
### Phase 4 — Boot root LAST
```powershell
.\run.ps1 -Port COM20 -Role root -Label node1 -Attack wormhole -Topology star -Wipe -Flash -Repeat 1
```
...
```powershell
cd tools
python export_logs.py --port COM20 --role child --label node5 --topology star --attack wormhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node6 --topology star --attack wormhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node4 --topology star --attack wormhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node3 --topology star --attack wormhole --repeat 1 --delete
python export_logs.py --port COM20 --role child --label node2 --topology star --attack wormhole --repeat 1 --delete
cd ..
```
> 🚨 **Carry each board back to its room but leave it UNPLUGGED.** A child powered
> now logs benign `layer = -1` rows all the way through Phase 7/8 — and those
...
```powershell
cd tools
python export_logs.py --port COM20 --role root --label node1 --topology star --attack wormhole --repeat 1
cd ..
Get-ChildItem tools\exports\wormhole\star_topology      # expect 7 files BEFORE analyzing
...
(Get-ChildItem tools\exports\wormhole\star_topology\trimmed\*.csv).Count   # MUST be 7 — a short count silently NaNs out PDR/latency
...
## 🃏 One-glance command card (star)

| Run | Children (COM26 / COM27 / COM25 / COM22 / COM21) | Root (COM20, LAST) | Export `--attack` | Extra |
|---|---|---|---|---|
| **Baseline** | all `-Topology star` | `-Role root -Topology star` | `none` | — |
| **Blackhole** | COM26 `-Attack blackhole -BlackholeRole attacker`; COM27/COM25/COM22/COM21 `-Attack blackhole -BlackholeRole victim` | `-Attack blackhole` | `blackhole` | Fig 4.17 note |
...
All flash commands carry `-Wipe -Flash`. Root **boots last**. Verify with `--expect star`.

---

## 🆘 Troubleshooting → shared table in [`LINEAR-RUNBOOK.md`](LINEAR-RUNBOOK.md#-quick-troubleshooting).
Star-specific: if `verify_topology --expect star` fails because a board went **2 hops**,
pull that board **closer to the root** so it attaches directly (most often **COM21** in
Master's Bedroom → move it to the Family-Hall doorway or nearer Bedroom 2), then re-run.
