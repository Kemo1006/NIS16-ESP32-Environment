# ESP32 Issues Log (Part 2 of 3) — older resolved issues

> ⬅️ **Back to [`esp32-issues.md`](esp32-issues.md)** (Part 1 of 3) — the
> active log (newest issues + the new-entry template). ➡️ Older still:
> [`esp32-issues-Part3.md`](esp32-issues-Part3.md) (Part 3 of 3, I-001…I-003).
> This file holds I-004…I-010, split out to keep the main log under 200 lines.
> Same Status legend (✅ FIXED / 🩹 WORKAROUND / 🔴 OPEN / ⛔ CAN'T FIX). Newest
> of this batch on top.
...
- **2026-07-13 addendum — cross-config ccache sharing:** each firmware variant
  gets its own `build_<role>_<attack>_<topology>` dir (see I-011), but that
  means the SAME source file compiled for `none` vs `blackhole` vs different
  topologies has a DIFFERENT absolute build path each time, which defeats
  ccache's fast "direct" hash mode (measured direct-hit rate was only ~28%).
  `run.ps1` now sets `CCACHE_BASEDIR` (repo root) and `CCACHE_SLOPPINESS`
  (`pch_defines,time_macros,include_file_mtime`) before invoking `idf.py`, so
  ccache treats build paths as relative and objects are reused ACROSS variants,
  not just within one. Also worth doing once per machine (not scripted, since
  it needs an elevated shell): add Windows Defender real-time exclusions for
  `C:\Espressif`, the ccache dir, and this repo folder — antivirus scanning of
  every `.o`/`.obj` write is the single biggest remaining cost on Windows.
...
## I-003 … I-001 · earliest issues → archived further

📦 Moved to **Part 3, [`esp32-issues-Part3.md`](esp32-issues-Part3.md)** (this
file grew past 200 lines): **I-003** `-lunwind` from wrong dir, **I-002** `could
not open COM9`, **I-001** export timeout workaround. Look there for those.
